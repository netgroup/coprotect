...
def sendDataToCloudProvider(dkg1, dkg2, CompanyPubKeyN, CompanyPubKeyE):
    ...
    sign = rsa.generateSign([str(CompanyPubKeyN), str(CompanyPubKeyE), str(share1), str(share2),
                             str(companyPubKeyShare)], Const.COMPANY)
    ...
    response = requests.post("http://"+Const.CLOUD_PROVIDER_ADDR+":"+Const.CLOUD_PROVIDER_PORT+"/"+Const.SHARES,
...
def computeFullShares(data, dkg1, dkg2, key, iv):
    ...
    sign = base64.decodestring(data[Const.SIGN])
    message = rsa.generateMessageForSign([str(CloudProviderPubKeyN), str(CloudProviderPubKeyE), str(other1),
                                          str(other2), str(pub)])
    # Verify signature
    if rsa.verifySign([CloudProviderPubKeyN, CloudProviderPubKeyE], message, sign) is True:
        ...
        # Create POST request
        data = json.dumps({Const.NE: CompanyPubKeyN, Const.E: CompanyPubKeyE,
                           Const.PROT_SHARE: base64.encodestring(protShare),
                           Const.SIGN: sign})
        headers = {'Content-Type': 'application/json'}
        response = requests.post("http://" + Const.CLOUD_PROVIDER_ADDR + ":" + Const.CLOUD_PROVIDER_PORT + "/" +
                                 Const.PROT_SHARE, data=data,
                                 headers=headers)
        if (response.content is Const.NO_METHOD) or (response.content is Const.BAD_REQ):
...
def password():
    if request.method == 'GET':
        ...
        message = (Const.getCurrentTime()+"COMPANY: Password of the protected shared private key fragment requested "
                                          "[ALLOWED ONLY FOR DEMONSTRATION PURPOSE]\n")
        sign = rsa.generateSign([str(Const.PSW), str(protectedIV)], Const.COMPANY)
        return json.dumps({Const.PASSWORD: Const.PSW, Const.IV: base64.encodestring(protectedIV), Const.LOG: message,
                           Const.SIGN: sign})
    else:
        return Const.NO_METHOD
